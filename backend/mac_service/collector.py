...
from app.services.consulta_processual.datajud import buscar_via_datajud  # noqa: E402
...
async def fetch_andamentos(cnj: str, session_data: dict | None) -> list:
    """Return andamentos for a CNJ number, newest first.

    Uses jus.br/PDPJ when a valid session is available, DataJud otherwise.
    ...
    if token:
        logger.info("Buscando via PDPJ (jus.br) — tribunal=%s", tribunal)
        andamentos = await buscar_via_pdpj(cnj, tribunal, token=token, session_data=session_data)
    else:
        logger.info("Buscando via DataJud (público) — tribunal=%s", tribunal)
        andamentos = await buscar_via_datajud(cnj, tribunal)
