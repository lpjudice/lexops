...
def _run_migrations() -> None:
    ...
    with engine.connect() as conn:
        ...
        conn.execute(text("ALTER TABLE contratos ADD COLUMN IF NOT EXISTS doc_gerado_filename VARCHAR(500)"))
        ...
        # Autos IA — índice de busca por tema (full-text). create_all() cria a tabela mas
        # não este índice funcional; em produção o Alembic já cuida disso.
        conn.execute(text(
            "CREATE INDEX IF NOT EXISTS ix_autos_ia_pecas_fts ON autos_ia_pecas "
...
app.include_router(contratos.router_publico)
