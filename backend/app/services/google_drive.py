...
def link_subpasta(nome_cliente: str, subfolder: str, sub_subfolder: str | None = None) -> str | None:
    """Retorna o link (webViewLink) de uma subpasta no Drive, criando se preciso."""
    tokens = _load_tokens()
    if not tokens:
        return None
    def _do(tkns):
    ...
    try:
        ...
        try:
            ...
            logger.warning("Falha ao obter link de subpasta: %s", exc)
            return None
...
def upload_arquivo_raiz(
    ...
    navegando/criando o caminho `subpath` (lista de nomes de pasta).
    ...
    try:
        ...
        if not _is_unauthorized(exc):
            logger.warning("Falha ao enviar arquivo (raiz) ao Drive: %s", exc)
            return None
        try:
            return _do(_refresh(tokens))
        except Exception as exc2:
            logger.warning("Falha ao enviar arquivo (raiz) ao Drive apos refresh: %s", exc2)
            return None
    ...
    try:
        ...
        logger.warning("Falha ao enviar arquivo (raiz) ao Drive apos refresh: %s", exc)
        return None
...
def criar_documento_google_raiz(nome_arquivo: str, subpath: list[str]) -> dict | None:
    """Cria um Google Doc em branco sob a raiz LexOps (mesmo esquema de pastas
    de upload_arquivo_raiz). Retorna {"id", "webViewLink"} ou None."""
    tokens = _load_tokens()
    if not tokens:
        return None
    ...
    try:
        ...
        if not _is_unauthorized(exc):
            logger.warning("Falha ao criar documento no Drive: %s", exc)
            return None
        tokens2 = _refresh(tokens)
        try:
            return _do(tokens2)
        except Exception as exc2:
            logger.warning("Falha ao criar documento no Drive apos refresh: %s", exc2)
            return None


def exportar_doc_como_pdf(file_id: str) -> bytes | None:
    """Exporta um Google Doc (nativo) como PDF pela Drive API. Diferente de
    baixar_arquivo_por_id: docs nativos não têm bytes próprios, só export."""
    tokens = _load_tokens()
    if not tokens:
        return None
    ...
    try:
        ...
        if not _is_unauthorized(exc):
            logger.warning("Falha ao exportar doc %s como PDF: %s", file_id, exc)
            return None
        tokens2 = _refresh(tokens)
        try:
            return _do(tokens2)
        except Exception as exc2:
            logger.warning("Falha ao exportar doc %s como PDF apos refresh: %s", file_id, exc2)
            return None
